...
def load_dataset():
        train_data_reader = DataReader(
            data_path='train-tfidf.txt', 
            batch_size=50, 
            vocab_size=vocab_size
            )
        test_data_reader = DataReader(
            data_path='test-tfidf.txt', 
            batch_size=50,
            vocab_size=vocab_size
        )
...
def save_parameters(name, value, epoch):
    filename = name.replace(':', '-colon-') + '-epoch-{}'.format(epoch)
...
def restore_parameters(name, epoch):
    ...
    if len(lines) == 1: #is a vector
        ...
        value = [[float(number) for number in lines[row].spit(',')]
...
if __name__ == '__main__':
    ...
    mlp = MLP(vocab_size, _hidden_size=50)
    predicted_labels, loss = mlp.build_graph()
    train_op = mlp.trainer(loss, learning_rate=0.1)
    ...
    test_data_reader = DataReader('test-tfidf.txt', 50, vocab_size)
    with tf.Session() as sess:        
        epoch = train_data_reader._num_epoch
        trainable_variables = tf.trainable_variables()
        for variable in trainable_variables:
