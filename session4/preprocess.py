...
def gen_data_and_vocab():
    ...
    parts = [path + dir_name + r'/' for dir_name in os.listdir(path)
    ...
    vocab = [word for word, freq in zip(word_count.keys(), word_count.values())
             if freq > 10]
    vocab.sort()
    with open('data/vocab-raw.txt', 'w') as f:
        f.write('\n'.join(vocab))
    ...
    with open('data/20news-train-raw.txt', 'w') as f:
        f.write('\n'.join(train_data))

    with open('data/20news-test-raw.txt', 'w') as f:
        f.write('\n'.join(test_data))
...
encode_data('data/20news-train-raw.txt', 'data/vocab-raw.txt')
encode_data('data/20news-test-raw.txt', 'data/vocab-raw.txt')