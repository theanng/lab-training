...
class DataReader:
    ...
    def next_batch(self):
        ...
        return self._data[start:end], self._labels[start:end], self._sentence_lengths[start:end], self._final_to
...
class RNN:
    ...
    def LSTM_layer(self, embeddings):
        ...
        lstm_outputs_sum = tf.math.reduce_sum(lstm_outputs_split, axis=1)
    ...
    def build_graph(self):
        ...
        logits = tf.linalg.matmul(lstm_outputs, weights) + biases
        ...
        predicted_labels = tf.math.argmax(probs, axis=1)
...
def train_and_evaluate_RNN():
    with open('../data/vocab_raw.txt') as f:
    ...
    with tf.Session() as sess:
        train_data_reader = DataReader(
            data_path='data/20news-train-encoded.txt',
            batch_size=50,
        )
        test_data_reader = DataReader(
            data_path='data/20news-test-encoded.txt',
            batch_size=50,
        )
        step = 0
        MAX_STEP = 100
